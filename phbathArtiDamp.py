...
class phbath:
    ...
    """
    def __init__(self,T,cats,debye,nw,dt,nmd,ml=None,mcof=2.0,sig=None,gamma=None,gwl=None,K00=None,K01=None,V01=None,eta_ad=0):
            ...
            if self.eta_ad != 0:
               ...
               for i in range(len(self.gwl)):
                    for it in range(self.kernel.shape[0]):
                        #gamnew[i,:,:]=gamnew[i,:,:]+2*self.gwl[i]*self.dt*self.kernel[it,:,:]*N.cos(self.gwl[i]*tl[it])
...
#--------------------------------------------------------------------------------------
    ...
    f=open("gam.dat","w")
    for i in range(len(eph.SigL)):
        f.write("%f   %f %f\n"%(eph.wl[i],\
        N.trace(phl.gamma[i]),-N.trace(eph.SigL[i].imag)/eph.wl[i]))
    f.close()
