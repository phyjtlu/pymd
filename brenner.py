...
class brenner:
    ...
    def __init__(self,xyz,anr,cell,devicedir=1,constrained=[],anr_constrained=[],lunit="Ang",eunit="eV"):#,constraints=[]
        ...
        self.anr_constrained = anr_constrained # type of extra atom fixed during MD.
        #1D array gives the coordinates of each atom
        #[xi,yi,zi],i=1,na
        self.xyz = xyz#N.array([a[1:] for a in xyz],dtype='d').flatten() # position vector
        self.anr=anr # atomic numbers (1=H,6=C,...)
        self.els = [PeriodicTable[a] for (a) in (anr)] #[a[0] for a in xyz] # Atom label (H,C,Au,...)
        ...
        self.devicedir=devicedir # direction to include large unitcell vacuum (1=x,2=y,3=z,0=none->use cell directly)
        self.lunit=lunit
        self.eunit=eunit
    ...
    def absforce(self,q):
        ...
        anr_b=N.concatenate((self.anr,self.anr_constrained))#[self.anr,self.anr_constrained]
        newR=N.concatenate((self.newx(q),self.constrained))#[self.newx(q),self.constrained]
        cell=self.cell
        if self.devicedir!=0:
           cell[self.devicedir-1]=self.cell[self.devicedir-1]+1000
        energy2,force,energy = py_brenner_module.brennerf(cell,anr_b[:],newR[:])
        force=force[0:len(q)]
